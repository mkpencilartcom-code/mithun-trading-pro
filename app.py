...
def scan_fno():
    ...
    for i, sym in enumerate(FNO):
        try:
            ...
            hd = (bh - c) / bh * 100
            if lu >= 1:
                up.append({"SYM": sym, "OPEN": round(o,2), "BODY_LOW": round(bl,2), "LTP": round(c,2), "LOW_UP %": round(lu,2)})
            if hd >= 1:
                down.append({"SYM": sym, "OPEN": round(o,2), "BODY_HIGH": round(bh,2), "LTP": round(c,2), "HIGH_DOWN %": round(hd,2)})
        except:
            pass
    ...
    for i, sym in enumerate(FNO):
        try:
            ...
            if df.empty or len(df)<20:
                bar.progress((i+1)/len(FNO)); continue
            ...
            b_df = day_df[day_df['Close']>or_h] if is_long else day_df[day_df['Close']<or_l]
            b_time = b_df.index[0].strftime("%H:%M") if not b_df.empty else "--:--"
            cards.append({"SYM":sym,"LTP":ltp,"ENTRY":entry,"SL":sl,"T1":t1,"T2":t2,"RANGE":range_pct,"TYPE":"Long Breakout" if is_long else "Short Breakdown","B_TIME":b_time,"DATE":last_day.strftime("%d %b"),"SECTOR":SECTOR_MAP_FULL.get(sym,"Others")})
        except:
            pass
        bar.progress((i+1)/len(FNO))
    bar.empty()
...
if menu == "FNO Scanner - 1%":
    ...
    if 'booster_cards' in st.session_state:
        cards = st.session_state['booster_cards']
        def time_sort_key(c):
            try:
                return datetime.strptime(c['B_TIME'],"%H:%M")
            except:
                return datetime.strptime("15:30","%H:%M")
        long_cards = sorted([x for x in cards if "Long" in x['TYPE']], key=time_sort_key)
        short_cards = sorted([x for x in cards if "Short" in x['TYPE']], key=time_sort_key)
        st.success(f"Total: {len(cards)} | LONG: {len(long_cards)} | SHORT: {len(short_cards)}")
        col_long, col_short = st.columns(2)
        with col_long:
            st.markdown(f"### LONG BODY - {len(long_cards)}")
            for c in long_cards:
                st.markdown(f"<div style='background:white;color:black;padding:12px;border-radius:12px;margin:8px 0;border-left:6px solid #00c853'><b>{c['SYM']} LONG BODY ⏰ {c['B_TIME']}</b><br>LTP {c['LTP']:.2f} ENTRY {c['ENTRY']:.2f} SL {c['SL']:.2f} T2 {c['T2']:.2f} RANGE {c['RANGE']:.2f}%</div>", unsafe_allow_html=True)
                draw_half_chart(c['SYM'], f"{c['TYPE']} {c['B_TIME']} RANGE {c['RANGE']:.2f}%")
        with col_short:
            st.markdown(f"### SHORT BODY - {len(short_cards)}")
            for c in short_cards:
                st.markdown(f"<div style='background:white;color:black;padding:12px;border-radius:12px;margin:8px 0;border-left:6px solid #d50000'><b>{c['SYM']} SHORT BODY ⏰ {c['B_TIME']}</b><br>LTP {c['LTP']:.2f} ENTRY {c['ENTRY']:.2f} SL {c['SL']:.2f} T2 {c['T2']:.2f} RANGE {c['RANGE']:.2f}%</div>", unsafe_allow_html=True)
                draw_half_chart(c['SYM'], f"{c['TYPE']} {c['B_TIME']} RANGE {c['RANGE']:.2f}%")
