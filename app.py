...
if menu == "📈 FNO - HALF CHART SIDE-BY-SIDE" and is_market_hours:
    ...
    st.markdown(f"<p style='text-align:center;color:#00d084;font-size:13px;margin-top:25px'>⏰ Next Candle: {next_time.strftime('%H:%M')} ({int(remaining)}s) | Last: {now.strftime('%H:%M:%S')}</p>", unsafe_allow_html=True)
    if st.session_state.get('df_u') is not None:
        time.sleep(max(1, min(remaining, 60)))
        st.rerun()
else:
    st.markdown(f"<p style='text-align:center;color:gray;font-size:12px;margin-top:20px'>Market: {'OPEN' if is_market_hours else 'Closed'} | {now.strftime('%H:%M:%S')} IST</p>", unsafe_allow_html=True)
