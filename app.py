...
if menu == "FNO Scanner - 1%":
    ...
    if 'df_h' in st.session_state and not st.session_state['df_h'].empty:
        ...
        for sec in sorted(df_show['SECTOR'].unique()):
            ...
            for idx, (_, row) in enumerate(sec_df.iterrows()):
                ...
                with cols[idx % 5]:
                    st.markdown(f"<div class='sector-box' style='background:{bg}'>{row['SYM']}<br>{row['CHANGE']}%<br>₹{row['LTP']:,.0f}</div>", unsafe_allow_html=True)
    ...
    if cards:
        def sort_key(c):
            try:
                h,m=map(int,str(c.get('SCANNER_TIME','15:30')).split(':')); return h*60+m
            except: return 9999
