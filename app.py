...
st.set_page_config(page_title="TradingPro FINAL - BOX+CHART+NEWS", layout="wide", initial_sidebar_state="expanded")
...
.chart-box { background: #131a28; border: 2px solid #1e2a3e; border-radius: 16px; padding: 15px; margin-bottom: 25px; }
</style>
""", unsafe_allow_html=True)

def draw_big_chart(symbol):
    try:
        ...
        if df.empty:
            st.error(f"{symbol} - Data nahi mila"); return
        df = df.tail(80)
        fig, ax = plt.subplots(figsize=(22, 12), facecolor='#0e121b')
        ...
        for i in range(len(df)):
            ...
            ax.plot([x,x],[l,h], color=col, linewidth=3)
            bh = abs(c-o)
            if bh < (h-l)*0.1: bh = (h-l)*0.1
            rect = Rectangle((x-0.0007, min(o,c)), 0.0014, bh, facecolor=col, edgecolor=col)
            ax.add_patch(rect)
        ax.set_title(f"{symbol} - 5 MIN CANDLE - 3X BIG", color='#00d084', fontsize=26, fontweight='bold', pad=20)
        ax.tick_params(colors='#aaa', labelsize=13)
...
def scan_fno():
    ...
    bar = st.progress(0, text="Scanning FNO 200 stocks...")
...
with st.sidebar:
    ...
    st.markdown("<span style='color:#00d084; font-weight:900'>● PREMIUM ACTIVE</span>", unsafe_allow_html=True)
    menu = st.radio("Navigation", ["📈 FNO - BOX + BIG CHARTS","📋 CUSTOM LIST + BIG CHARTS","📊 Sector Heatmap","📰 NEWS Terminal - 87 Sources"], label_visibility="collapsed")
    st.divider()
    st.caption(f"📅 {datetime.now(IST).strftime('%d %b %Y %I:%M %p')} IST")

if menu == "📈 FNO - BOX + BIG CHARTS":
    st.title("📈 FNO - BOX me List + Niche Bade Charts")
    if st.button("🚀 SCAN FNO", type="primary", use_container_width=True):
        df_u, df_d, last_dt = scan_fno()
        st.info(f"Last Trading Date: {last_dt} | Upar BOX me list, niche bade charts")
        c1,c2 = st.columns(2)
        with c1:
            st.markdown(f"<div class='green-box'>↗ LOW se 1% UP: {len(df_u)}</div>", unsafe_allow_html=True)
            up_str = ", ".join(df_u['SYM'].tolist()) if not df_u.empty else "No data"
            st.text_area("📦 LIST - LOW UP BOX", value=up_str, height=130, key="up_box")
            st.dataframe(df_u.sort_values("LOW_UP %", ascending=False) if not df_u.empty else df_u, use_container_width=True)
        with c2:
            st.markdown(f"<div class='red-box'>↘ HIGH se 1% DOWN: {len(df_d)}</div>", unsafe_allow_html=True)
            down_str = ", ".join(df_d['SYM'].tolist()) if not df_d.empty else "No data"
            st.text_area("📦 LIST - HIGH DOWN BOX", value=down_str, height=130, key="down_box")
            st.dataframe(df_d.sort_values("HIGH_DOWN %", ascending=False) if not df_d.empty else df_d, use_container_width=True)

        st.divider()
        st.header("📊 Niche sabka bada chart - 3X BIG")
        tab1, tab2 = st.tabs([f"LOW UP Charts ({len(df_u)})", f"HIGH DOWN Charts ({len(df_d)})"])
        with tab1:
            if not df_u.empty:
                for sym in df_u.sort_values("LOW_UP %", ascending=False)['SYM'].tolist():
                    with st.container(border=True):
                        st.markdown(f"### 📈 {sym}")
                        draw_big_chart(sym)
        with tab2:
            if not df_d.empty:
                for sym in df_d.sort_values("HIGH_DOWN %", ascending=False)['SYM'].tolist():
                    with st.container(border=True):
                        st.markdown(f"### 📉 {sym}")
                        draw_big_chart(sym)

elif menu == "📋 CUSTOM LIST + BIG CHARTS":
    st.title("📋 CUSTOM BOX - List daalo, niche bade charts dekho")
    default_list = "PATANJALI, MANKIND, LICHSGFIN, RELIANCE, TCS, INFY, HDFCBANK"
    user_input = st.text_area("📦 STOCK LIST BOX (comma se)", value=default_list, height=150)
    if st.button("🔥 BIG CHARTS DIKHAO", type="primary", use_container_width=True):
        symbols = [s.strip().upper().replace(".NS","") for s in user_input.replace("\n", ",").split(",") if s.strip()!=""]
        symbols = list(dict.fromkeys(symbols))
        st.success(f"{len(symbols)} stocks")
        st.text_area("Final List", value=", ".join(symbols), height=80)
        for sym in symbols:
            with st.container(border=True):
                st.markdown(f"<h2 style='text-align:center; color:#00d084;'>📈 {sym}</h2>", unsafe_allow_html=True)
                draw_big_chart(sym)
    ...
    if 'df_h' in st.session_state and not st.session_state['df_h'].empty:
        ...
        fig_bar=px.bar(sec_perf,x='SECTOR',y='CHANGE',color='CHANGE',color_continuous_scale=[(0,"#d50000"),(0.5,"#1e222d"),(1,"#00c853")],text=sec_perf['CHANGE'].round(2).astype(str)+'%',range_color=[-3,3])
        ...
        if clicked: st.session_state['selected_sector']=clicked[0]['x']
        if st.session_state.get('selected_sector'):
            ...
            st.success(f"👉 {sel} - {len(sdf)} Stocks")
